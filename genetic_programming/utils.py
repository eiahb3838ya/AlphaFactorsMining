...
def rowwise_cov(factor, shiftedReturn):
    try:
        if isinstance(factor, GeneralData):
            validFactor = np.ma.masked_invalid(factor.generalData)
        else:
            validFactor = np.ma.masked_invalid(factor)
        if isinstance(shiftedReturn, GeneralData):
            validShiftedReturn = np.ma.masked_invalid(shiftedReturn.generalData)
        else:
    ...
    toReturn = np.ma.dot(validFactor_dm, validShiftedReturn_dm.T).diagonal()

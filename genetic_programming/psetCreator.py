...
def pset_creator(materialDataNames):
    ...
    # for aName, primitive in [o for o in getmembers(coupleDataNFunctions) if isfunction(o[1])]:
    # print('add primitive from {:<20}: {}'.format('coupleDataFunctions', aName))
    pset.addPrimitive(coupleDataNFunctions.ts_corr, [GeneralData, GeneralData, int], GeneralData, 'ts_corr')
    pset.addPrimitive(coupleDataNFunctions.ts_cov, [GeneralData, GeneralData, int], GeneralData, 'ts_cov')
